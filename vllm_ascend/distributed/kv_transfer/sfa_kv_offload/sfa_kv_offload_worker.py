...
class SFAKVOffloadWorker:
    ...
    def register_kv_caches(self, kv_caches: dict[str, torch.Tensor]):
        ...
        if self.use_sparse and self.use_offload:
            ...
            self.gvas_buffer_cpu = empty_tensor([self.max_num_reqs * self.sfa_sparse_topk * 2], dtype=torch.int64, pin_memory=True)
            self.addr_buffer_cpu = empty_tensor([self.max_num_reqs * self.sfa_sparse_topk * 2], dtype=torch.int64, pin_memory=True)
            self.size_buffer_cpu = empty_tensor([self.max_num_reqs * self.sfa_sparse_topk * 2], dtype=torch.int32, pin_memory=True)
            self.num_tokens_buffer_cpu = empty_tensor([1], dtype=torch.int32, pin_memory=True)

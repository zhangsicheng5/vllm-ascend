...
from vllm.distributed.parallel_state import get_context_model_parallel_rank, get_context_model_parallel_world_size
...
class LLMDataDistCMgrConnectorScheduler():
    ...
    def __init__(self, vllm_config: VllmConfig, engine_id: Optional[str]):
        ...
        self.sp_size = tp_size if self.enable_sp else 1

        self.port = dp_rank_local * self.cp_size * tp_size + envs.VLLM_LLMDD_RPC_PORT if dp_rank_local is not None else tp_size + envs.VLLM_LLMDD_RPC_PORT
...
class LLMDataDistCMgrConnectorWorker():
  ...
  """
    ...
    def __init__(self, vllm_config: VllmConfig):
        ...
        self.sp_size = self.tp_size if self.enable_sp else 1
    ...
    def listen_for_agent_metadata_req(self, event: threading.Event):
        ...
        port = envs.VLLM_LLMDD_RPC_PORT + self.local_dp_rank * self.cp_size * self.tp_size + self.cp_rank * self.tp_size + self.tp_rank \
            if self.local_dp_rank is not None else envs.VLLM_LLMDD_RPC_PORT + self.tp_size + self.tp_rank
    ...
    def _read_blocks(
        ...
        self.send_finish_to_remote(remote_ip, remote_ports[0], request_id)
        with self.thread_lock:

...
class NPUModelRunner(GPUModelRunner):
    ...
    def _prepare_inputs(
        ...
        if torch.distributed.get_rank() == 0:
            logger.info(f'>>>>> input tokens = {num_scheduled_tokens}')
            logger.info(f'>>>>> input req_ids = {self.input_batch.req_ids}')
        ...
        if self.use_offload:
            self.num_offloaded_blocks = torch.tensor(
                # np.maximum(self.input_batch.num_computed_tokens_cpu[:num_reqs] // self.block_size - 1, 0),

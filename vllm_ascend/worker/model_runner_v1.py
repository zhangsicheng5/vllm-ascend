...
class NPUModelRunner(GPUModelRunner):
            ...
            for row, req in enumerate(self.input_batch.req_ids[:num_reqs]):
                if req not in self._offload_request_slots:
                    ...
                    if computed_all is not None:
                        computed = int(computed_all[row])
                        if self._nano_needs_dense_fill(computed):
                            dense_fills[slot] = (row, computed)
    ...
    def _nano_needs_dense_fill(self, computed_tokens: int) -> bool:
        # Only the colocate-debug path stages prefill KV in the NPU paged
        # cache + CPU pool; a PD node receives the dense row directly via the
        # connector D2D and must not re-copy it.
        return bool(
            self.sparse_kv_offload_config is not None
            and self.sparse_kv_offload_config.keep_device_kv_cache
            and 0 < computed_tokens <= self.sparse_kv_offload_config.topk_buffer_size
        )
    ...
    def _dense_fill_nano_rows(self, dense_fills: dict[int, tuple[int, int]]) -> None:
        ...
        copy-SFA's dense (-3) mode reads the row front-to-back, so a fresh
        short request needs its full prefix resident before the first decode
        forward. Runs outside the captured graph at metadata-build time; the
        per-layer host/device bases were pinned by ``bind_nano_kv_cache``.
        """

...
class AscendUnquantizedFusedMoEMethod(UnquantizedFusedMoEMethod):
    ...
    def process_weights_after_loading(self, layer):
        ...
        if enable_fused_mc2:
            # MegaMoe is MXFP-only (FP8/FP4), so the unquantized path always
            # uses the dispatch_ffn_combine op with FRACTAL_NZ weights.
    ...
    def apply(
        ...
        if _EXTRA_CTX.moe_comm_type == MoECommType.FUSED_MC2:
            # MegaMoe is MXFP-only, so unquantized BF16 MoE always uses the
            # dispatch_ffn_combine path with empty scale/bias placeholders.

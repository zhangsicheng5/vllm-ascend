...
class SPColumnParallelOp(CustomColumnParallelOp):
    ...
    def apply(
        ...
        """Linear layer with column parallelism for sequence parallel.
        All gather sequence before matmul.
        We should only do all-gather while prefill.
        We should only do all-gather in gate_up_proj,
        (for mla q_b_proj and kv_b_proj, we need to do all-gather before storing kv_cache in MLAImpl)
        """
        ...
        if self.enable_sp and is_prefill and 'b_proj' not in self.prefix:
            input_ = get_tp_group().all_gather(input_.contiguous(), 0)
...
class SPRowParallelOp(CustomRowParallelOp):

    def __init__(self, layer):
        super().__init__(layer)

    def apply(self, input_: torch.Tensor) -> torch.Tensor:
        """Linear layer with row parallelism for sequence parallel.
        Reduce scatter sequence after matmul.
        We should only replace all-reduce to reduce-scatter while prefill.
        """
...
def get_column_parallel_op(
    ...
    if "gate_up_proj" in prefix and mlp_tp_enable() and not is_sp_enabled():
        ...
        custom_op = SPColumnParallelOp(layer, prefix)
...
def get_row_parallel_op(
    ...
    if "down_proj" in prefix and mlp_tp_enable() and not is_sp_enabled():
        ...
        custom_op = SPRowParallelOp(layer)

...
def _run_numpy_bridge():
    ...
    old = np.array(
        [[
            3 + 3 * REQ_ID_OFFSET_STRIDE,
            7 + 3 * REQ_ID_OFFSET_STRIDE,
            15 + 3 * REQ_ID_OFFSET_STRIDE,
            22 + 3 * REQ_ID_OFFSET_STRIDE,
            -1,
            -1,
        ]],
        dtype=np.int64,
...
def _run_torch_bridge():
    ...
    update_topk_indices_cpu(req_ids_cpu, old_cpu, new_cpu, workspace)

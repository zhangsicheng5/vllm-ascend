...
#include "../op_kernel/sfa_impl/sparse_tail_attention_template_tiling_key.h"
...
const std::map<std::string, std::vector<CopySfaMtpSFALayout>> LAYOUT_SUPPORT_MAP = {
    {QUERY_NAME,             {CopySfaMtpSFALayout::BSND, CopySfaMtpSFALayout::TND}},
    {KEY_NAME,               {CopySfaMtpSFALayout::BSND, CopySfaMtpSFALayout::TND, CopySfaMtpSFALayout::PA_BSND}},
    {VALUE_NAME,             {CopySfaMtpSFALayout::BSND, CopySfaMtpSFALayout::TND, CopySfaMtpSFALayout::PA_BSND}},
    {ATTEN_OUT_NAME,         {CopySfaMtpSFALayout::BSND, CopySfaMtpSFALayout::TND}},
};
...
struct CopySfaMtpAttentionCompileInfo {
    int64_t core_num;
};

static const std::map<CopySfaMtpSFALayout, std::vector<CopySfaMtpSFAAxis>> SFA_LAYOUT_AXIS_MAP = {
    {CopySfaMtpSFALayout::BSND, {CopySfaMtpSFAAxis::B, CopySfaMtpSFAAxis::S, CopySfaMtpSFAAxis::N, CopySfaMtpSFAAxis::D}},
    {CopySfaMtpSFALayout::TND, {CopySfaMtpSFAAxis::T, CopySfaMtpSFAAxis::N, CopySfaMtpSFAAxis::D}},
    {CopySfaMtpSFALayout::PA_BSND, {CopySfaMtpSFAAxis::Bn, CopySfaMtpSFAAxis::Bs, CopySfaMtpSFAAxis::N, CopySfaMtpSFAAxis::D}},
};

static const std::map<CopySfaMtpSFALayout, size_t> SFA_LAYOUT_DIM_MAP = {
    {CopySfaMtpSFALayout::BSND, DIM_NUM_FOUR},
    {CopySfaMtpSFALayout::TND, DIM_NUM_THREE},
    {CopySfaMtpSFALayout::PA_BSND, DIM_NUM_FOUR},
};
...
static std::string SFADataTypeToSerialString(ge::DataType type)
{
    ...
    if (it != DATATYPE_TO_STRING_MAP.end()) {
        ...
        OPS_LOG_E("CopySfaMtpAttention", "datatype %d not support", type);
        return "UNDEFINED";
    }
}

string CopySfaMtpTensorDesc2String(const gert::StorageShape *shape, const gert::CompileTimeTensorDesc *tensor)
{
...
string CopySfaMtpDebugTilingContext(const gert::TilingContext *context)
{
    std::ostringstream oss;
    for (size_t i = 0; i < context->GetComputeNodeInfo()->GetInputsNum(); ++i) {
        oss << "input" << i << ": ";
        oss << CopySfaMtpTensorDesc2String(context->GetInputShape(i), context->GetInputDesc(i));
    }

    for (size_t i = 0; i < context->GetComputeNodeInfo()->GetOutputsNum(); ++i) {
        oss << "output" << i << ": ";
        oss << CopySfaMtpTensorDesc2String(context->GetOutputShape(i), context->GetOutputDesc(i));
    }
    return oss.str();
}
...
{
    switch (layout) {
        case CopySfaMtpSFALayout::BSND: return "BSND";
        case CopySfaMtpSFALayout::TND: return "TND";
        case CopySfaMtpSFALayout::PA_BSND: return "PA_BSND";
        default: return "UNKNOWN";
    }
}

ge::graphStatus CopySfaMtpSFAMlaTiling::SetBlockDim(uint32_t blockDim)
{
    context_->SetBlockDim(blockDim);
    return ge::GRAPH_SUCCESS;
}

ge::graphStatus CopySfaMtpSFAMlaTiling::SetTilingKey(uint64_t tilingKey)
{
    context_->SetTilingKey(tilingKey);
    return ge::GRAPH_SUCCESS;
}

ge::graphStatus CopySfaMtpSFAMlaTiling::SetWorkspaceSize(uint64_t workspaceSize)
{
...
ge::graphStatus CopySfaMtpSFAMlaTiling::SetTilingData(TilingDef &tilingData)
{
...
ge::graphStatus CopySfaMtpSFAMlaTiling::GetPlatformInfo()
{
...
void CopySfaMtpSFAMlaTiling::GenTilingKey()
{
    tilingKey_ = GET_TPL_TILING_KEY(0U, static_cast<uint32_t>(CopySfaMtpSFALayout::TND),
                                   static_cast<uint32_t>(CopySfaMtpSFALayout::PA_BSND), 1U);

    OPS_LOG_I(sfaInfo_->opName, "SFA tilingKey_: %lu.", tilingKey_);
}

void CopySfaMtpSFAMlaTiling::ZeroTensorProcess()
{
...
void CopySfaMtpSFAMlaTiling::InitParams()
{
    perfMode_ = CopySfaMtpSFAPerfMode::V_TEMPLATE_MODE;
...
void CopySfaMtpSFAMlaTiling::CalcUbBmm()
{
...
void CopySfaMtpSFAMlaTiling::CheckUbSpace()
{
    CalcUbBmm();
}

void CopySfaMtpSFAMlaTiling::CalcInnerSize(uint32_t s2Size)
{
    sInnerSize_ = 512;
    if (splitKVFlag_ && sfaInfo_->qLayout != CopySfaMtpSFALayout::TND) {
        if (s2Size == 256) {
            sInnerSize_ = 128;
...
void CopySfaMtpSFAMlaTiling::SplitBalanced()
{
    CalcInnerSize(sfaInfo_->s2Size);

    CopySfaMtpInnerSplitParams innerSplitParams;
...
void CopySfaMtpSFAMlaTiling::Split()
{
    SplitBalanced();
}

void CopySfaMtpSFAMlaTiling::FillTilingBaseParamsMla()
{
...
// for flash decode
void CopySfaMtpSFAMlaTiling::FillTilingSplitKVMla()
{
...
void CopySfaMtpSFAMlaTiling::FillTilingSingleCoreParamsMla()
{
    tilingData_.singleCoreParams.set_usedCoreNum(usedCoreNum_);
}

void CopySfaMtpSFAMlaTiling::FillTilingSingleCoreTensorSizeMla()
{
...
void CopySfaMtpSFAMlaTiling::FillTiling()
{
...
uint32_t CopySfaMtpSFAMlaTiling::CalcBalanceFDParamNums(const uint32_t actCoreNum)
{
    return actCoreNum * 2 * sfaInfo_->n2Size * mBaseSize_;
}

void CopySfaMtpSFAMlaTiling::NormalCalcFDWorkSpace(const uint32_t actCoreNum)
{
    if (splitKVFlag_) {
...
void CopySfaMtpSFAMlaTiling::CalcFDWorkSpace(const uint32_t actCoreNum)
{
    NormalCalcFDWorkSpace(actCoreNum);
}

void CopySfaMtpSFAMlaTiling::GetWorkspaceSize()
{
...
void CopySfaMtpSFAMlaTiling::CalcBlockDim()
{
    auto ascendcPlatform = platform_ascendc::PlatformAscendC(sfaInfo_->platformInfo);
...
ge::graphStatus CopySfaMtpSFAMlaTiling::DoOpTiling(CopySfaMtpSFATilingInfo *sfaInfo)
{
    sfaInfo_ = sfaInfo;
...
ge::graphStatus TilingCopySfaMtpAttention(gert::TilingContext *context)
{
    CopySfaMtpSFATilingInfo sfaInfo;
    CopySfaMtpSFAInfoParser sfaInfoParser(context);
    if (sfaInfoParser.Parse(sfaInfo) != ge::GRAPH_SUCCESS) {
        return ge::GRAPH_FAILED;
    }

    CopySfaMtpSFATilingCheck tilingChecker(sfaInfo);
    if (tilingChecker.Process() != ge::GRAPH_SUCCESS) {
        return ge::GRAPH_FAILED;
    }

    CopySfaMtpSFAMlaTiling tiling(context);
    return tiling.DoOpTiling(&sfaInfo);
}

ge::graphStatus TilingCopySfaMtpAttentionMtp(gert::TilingContext *context)
{
    CopySfaMtpSFATilingInfo sfaInfo;
    CopySfaMtpSFAInfoParser sfaInfoParser(context);
    if (sfaInfoParser.Parse(sfaInfo) != ge::GRAPH_SUCCESS) {
    ...
    // validation as standardized COPYSFA-MTP: B <= T <= 16B.
    CopySfaMtpSFAMlaTiling tiling(context);
    ...
    if (status == ge::GRAPH_SUCCESS) {
        ...
        // Match the official SFA batch scheduler for one AIC plus two AIVs.
        context->SetScheduleMode(1);
    }
    return status;
}

ge::graphStatus TilingPrepareForCopySfaMtpAttention(gert::TilingParseContext *context)
{
    (void)context;
    return ge::GRAPH_SUCCESS;
}

ge::graphStatus CopySfaMtpSFATilingCheck::GetExpectedShape(gert::Shape &shapeExpected,
    const CopySfaMtpSFATilingShapeCompareParam &param, const CopySfaMtpSFALayout &layout) const
{
    if (layout == CopySfaMtpSFALayout::BSND) {
        shapeExpected = gert::Shape({param.B, param.S, param.N, param.D});
    } else if (layout == CopySfaMtpSFALayout::TND) {
        shapeExpected = gert::Shape({param.T, param.N, param.D});
    } else if (layout == CopySfaMtpSFALayout::PA_BSND) {
...
ge::graphStatus CopySfaMtpSFATilingCheck::CompareShape(CopySfaMtpSFATilingShapeCompareParam &param,
...
void CopySfaMtpSFATilingCheck::LogErrorDtypeSupport(const std::vector<ge::DataType> &expectDtypeList,
...
ge::graphStatus CopySfaMtpSFATilingCheck::CheckDtypeSupport(const gert::CompileTimeTensorDesc *desc,
...
template <typename T>
void CopySfaMtpSFATilingCheck::LogErrorNumberSupport(const std::vector<T> &expectNumberList,
...
template <typename T>
void CopySfaMtpSFATilingCheck::LogErrorDimNumSupport(const std::vector<T> &expectNumberList,
    const T &actualValue, const std::string &name) const
{
    LogErrorNumberSupport(expectNumberList, actualValue, name, "dimension");
}

ge::graphStatus CopySfaMtpSFATilingCheck::CheckDimNumInLayoutSupport(const CopySfaMtpSFALayout &layout,
...
ge::graphStatus CopySfaMtpSFATilingCheck::CheckDimNumSupport(const gert::StorageShape *shape,
...
void CopySfaMtpSFATilingCheck::LogErrorLayoutSupport(const std::vector<CopySfaMtpSFALayout> &expectLayoutList,
...
ge::graphStatus CopySfaMtpSFATilingCheck::CheckLayoutSupport(const CopySfaMtpSFALayout &actualLayout, const std::string &name) const
{
...
ge::graphStatus CopySfaMtpSFATilingCheck::CheckSingleParaQuery() const
{
...
ge::graphStatus CopySfaMtpSFATilingCheck::CheckSingleParaKey() const
{
...
ge::graphStatus CopySfaMtpSFATilingCheck::CheckSingleParaNumHeads() const
{
    return ge::GRAPH_SUCCESS;
}

ge::graphStatus CopySfaMtpSFATilingCheck::CheckSingleParaKvHeadNums() const
{
    return ge::GRAPH_SUCCESS;
}

ge::graphStatus CopySfaMtpSFATilingCheck::CheckSingleParaSparseMode() const
{
...
ge::graphStatus CopySfaMtpSFATilingCheck::CheckSingleParaSparseBlockSize() const
{
...
ge::graphStatus CopySfaMtpSFATilingCheck::CheckSingleParaSparseIndices() const
{
...
ge::graphStatus CopySfaMtpSFATilingCheck::CheckSinglePara() const
{
...
ge::graphStatus CopySfaMtpSFATilingCheck::CheckRopeExistence()
{
...
ge::graphStatus CopySfaMtpSFATilingCheck::CheckExists(const void *pointer, const std::string &name) const
{
...
ge::graphStatus CopySfaMtpSFATilingCheck::CheckNotExists(const void *pointer, const std::string &name) const
{
    OPS_ERR_IF(pointer != nullptr,
...
ge::graphStatus CopySfaMtpSFATilingCheck::CheckExistsByMap(const std::map<std::string, const void *> &paramMap) const
{
...
ge::graphStatus CopySfaMtpSFATilingCheck::CheckNotExistsByMap(const std::map<std::string, const void *> &paramMap) const
{
    for (const auto& kv : paramMap) {
...
ge::graphStatus CopySfaMtpSFATilingCheck::CheckExistenceByMap(std::map<std::string, const void *> &existMap,
...
template <typename T>
ge::graphStatus CopySfaMtpSFATilingCheck::CheckAttrValueByMap(std::map<std::string, std::pair<const T *, T>> &attrMap) const
{
...
ge::graphStatus CopySfaMtpSFATilingCheck::CheckParaExistenceMlaNoquant() const
{
    if (kvStorageMode_ != CopySfaMtpKvStorageMode::PAGE_ATTENTION) {
        return ge::GRAPH_SUCCESS;
    }
...
ge::graphStatus CopySfaMtpSFATilingCheck::CheckParaExistenceMla() const
{
    return CheckParaExistenceMlaNoquant();
}

ge::graphStatus CopySfaMtpSFATilingCheck::CheckParaExistence()
{
...
ge::graphStatus CopySfaMtpSFATilingCheck::GetActualSeqLenSize(uint32_t &size, const gert::Tensor *tensor,
    const CopySfaMtpSFALayout &layoutQuery, const std::string &name)
{
    if (tensor == nullptr) {
...
void CopySfaMtpSFATilingCheck::SetSFAShapeCompare()
{
...
ge::graphStatus CopySfaMtpSFATilingCheck::CheckBlockTable() const
{
    if (kvStorageMode_ != CopySfaMtpKvStorageMode::PAGE_ATTENTION) {
...
ge::graphStatus CopySfaMtpSFATilingCheck::CheckDTypeConsistency(const ge::DataType &actualDtype,
...
ge::graphStatus CopySfaMtpSFATilingCheck::CheckQRopeShape()
{
    CopySfaMtpSFATilingShapeCompareParam shapeParams;
...
ge::graphStatus CopySfaMtpSFATilingCheck::CheckTopkShape()
{
    CopySfaMtpSFATilingShapeCompareParam shapeParams;
    shapeParams.B = bSize_;
...
ge::graphStatus CopySfaMtpSFATilingCheck::CheckAttenOutShape()
{
    CopySfaMtpSFATilingShapeCompareParam shapeParams;
    shapeParams.B = bSize_;
    shapeParams.N = n1Size_;
...
ge::graphStatus CopySfaMtpSFATilingCheck::CheckAttenOut()
{
...
ge::graphStatus CopySfaMtpSFATilingCheck::CheckQRope()
{
...
ge::graphStatus CopySfaMtpSFATilingCheck::CheckTopK()
{
...
ge::graphStatus CopySfaMtpSFATilingCheck::CheckVAndKRopeShapeForBatchContinuous()
{
    CopySfaMtpSFATilingShapeCompareParam shapeParams;
    shapeParams.B = bSize_;
    shapeParams.N = n2Size_;
...
uint32_t CopySfaMtpSFATilingCheck::GetTypeSize(ge::DataType dtype) const
{
...
ge::graphStatus CopySfaMtpSFATilingCheck::CheckVAndKRopeShapeForPageAttention()
{
...
ge::graphStatus CopySfaMtpSFATilingCheck::CheckVAndKRopeShape()
{
    if (kvStorageMode_ == CopySfaMtpKvStorageMode::BATCH_CONTINUOUS) {
...
ge::graphStatus CopySfaMtpSFATilingCheck::CheckVAndKRope()
{
...
ge::graphStatus CopySfaMtpSFATilingCheck::CheckActualSeqLensQ()
{
...
ge::graphStatus CopySfaMtpSFATilingCheck::CheckActualSeqLensQDType()
{
...
ge::graphStatus CopySfaMtpSFATilingCheck::CheckActualSeqLensQShape()
{
    if (opParamInfo_.actualSeqLengthsQ.tensor == nullptr) {
        return ge::GRAPH_SUCCESS;
    }
...
ge::graphStatus CopySfaMtpSFATilingCheck::CheckActualSeqLens()
{
...
ge::graphStatus CopySfaMtpSFATilingCheck::CheckActualSeqLensDType()
{
...
ge::graphStatus CopySfaMtpSFATilingCheck::CheckActualSeqLensShape()
{
    if (opParamInfo_.actualSeqLengths.tensor == nullptr) {
        return ge::GRAPH_SUCCESS;
    }
...
ge::graphStatus CopySfaMtpSFATilingCheck::CheckMultiParaConsistency()
{
    SetSFAShapeCompare();
...
ge::graphStatus CopySfaMtpSFATilingCheck::CheckFeatureMlaNoQuantShape() const
{
    OPS_ERR_IF(bSize_ <= 0,
        OPS_LOG_E(opName_, "batch_size should be greater than 0, but got %u", bSize_),
        return ge::GRAPH_FAILED);

    OPS_ERR_IF(qTSize_ <= 0 && (qLayout_ == CopySfaMtpSFALayout::TND),
...
ge::graphStatus CopySfaMtpSFATilingCheck::CheckFeatureMlaNoQuantLayout() const
{
...
ge::graphStatus CopySfaMtpSFATilingCheck::CheckFeatureMlaNoQuantDtype() const
{
...
ge::graphStatus CopySfaMtpSFATilingCheck::CheckFeatureMlaNoquantPa() const
{
    if (kvStorageMode_ != CopySfaMtpKvStorageMode::PAGE_ATTENTION) {
        return ge::GRAPH_SUCCESS;
    }
...
ge::graphStatus CopySfaMtpSFATilingCheck::CheckFeatureMlaNoquant() const
{
...
ge::graphStatus CopySfaMtpSFATilingCheck::CheckFeatureMla() const
{
    return CheckFeatureMlaNoquant();
}

ge::graphStatus CopySfaMtpSFATilingCheck::CheckFeature() const
{
    return CheckFeatureMla();
}

ge::graphStatus CopySfaMtpSFATilingCheck::CheckDecodeOnly() const
{
    OPS_ERR_IF(qLayout_ != CopySfaMtpSFALayout::TND || kvLayout_ != CopySfaMtpSFALayout::PA_BSND,
...
void CopySfaMtpSFATilingCheck::Init()
{
...
ge::graphStatus CopySfaMtpSFATilingCheck::Process()
{
    Init();
...
bool CopySfaMtpSFAInfoParser::HasAxis(const CopySfaMtpSFAAxis &axis, const CopySfaMtpSFALayout &layout, const gert::Shape &shape) const
{
    ...
    const std::vector<CopySfaMtpSFAAxis>& axes = layoutIt->second;
...
size_t CopySfaMtpSFAInfoParser::GetAxisIdx(const CopySfaMtpSFAAxis &axis, const CopySfaMtpSFALayout &layout) const
{
    const std::vector<CopySfaMtpSFAAxis>& axes = SFA_LAYOUT_AXIS_MAP.find(layout)->second;
    const auto& axisIt = std::find(axes.begin(), axes.end(), axis);
    return std::distance(axes.begin(), axisIt);
}

uint32_t CopySfaMtpSFAInfoParser::GetAxisNum(const gert::Shape &shape, const CopySfaMtpSFAAxis &axis,const CopySfaMtpSFALayout &layout) const
{
    return HasAxis(axis, layout, shape) ? shape.GetDim(GetAxisIdx(axis, layout)) : invalidDimValue_;
}

ge::graphStatus CopySfaMtpSFAInfoParser::CheckRequiredInOutExistence() const
{
...
ge::graphStatus CopySfaMtpSFAInfoParser::CheckRequiredAttrExistence() const
{
...
ge::graphStatus CopySfaMtpSFAInfoParser::CheckRequiredParaExistence() const
{
...
ge::graphStatus CopySfaMtpSFAInfoParser::GetActualSeqLenSize(uint32_t &size, const gert::Tensor *tensor,
...
ge::graphStatus CopySfaMtpSFAInfoParser::GetActualSeqLenQSize(uint32_t &size)
{
    return GetActualSeqLenSize(size, opParamInfo_.actualSeqLengthsQ.tensor, qLayout_, "actualSeqLengthsQ");
}

ge::graphStatus CopySfaMtpSFAInfoParser::GetOpName()
{
    if (context_->GetNodeName() == nullptr) {
        OPS_LOG_E("CopySfaMtpAttention", "opName got from TilingContext is nullptr");
        return ge::GRAPH_FAILED;
    }
...
ge::graphStatus CopySfaMtpSFAInfoParser::GetNpuInfo()
{
...
void CopySfaMtpSFAInfoParser::GetOptionalInputParaInfo()
{
...
void CopySfaMtpSFAInfoParser::GetInputParaInfo()
{
...
void CopySfaMtpSFAInfoParser::GetOutputParaInfo()
{
...
ge::graphStatus CopySfaMtpSFAInfoParser::GetAttrParaInfo()
{
...
ge::graphStatus CopySfaMtpSFAInfoParser::GetOpParaInfo()
{
    GetInputParaInfo();
    GetOutputParaInfo();
...
ge::graphStatus CopySfaMtpSFAInfoParser::GetInOutDataType()
{
...
ge::graphStatus CopySfaMtpSFAInfoParser::GetBatchSize()
{
    if (qLayout_ == CopySfaMtpSFALayout::TND) {
        return GetActualSeqLenQSize(bSize_);
    } else { // BSND
        bSize_ = GetAxisNum(queryShape_, CopySfaMtpSFAAxis::B, qLayout_);
        return ge::GRAPH_SUCCESS;
    }
}

ge::graphStatus CopySfaMtpSFAInfoParser::GetQTSize()
{
    qTSize_ = (qLayout_ == CopySfaMtpSFALayout::TND) ? GetAxisNum(queryShape_, CopySfaMtpSFAAxis::T, qLayout_) : 0;
    return ge::GRAPH_SUCCESS;
}

ge::graphStatus CopySfaMtpSFAInfoParser::GetKVTSize()
{
    kvTSize_ = (kvLayout_ == CopySfaMtpSFALayout::TND) ? GetAxisNum(keyShape_, CopySfaMtpSFAAxis::T, kvLayout_) : 0;
    return ge::GRAPH_SUCCESS;
}

ge::graphStatus CopySfaMtpSFAInfoParser::GetQkHeadDim()
{
    qkHeadDim_ = GetAxisNum(queryShape_, CopySfaMtpSFAAxis::D, qLayout_);
    return ge::GRAPH_SUCCESS;
}

ge::graphStatus CopySfaMtpSFAInfoParser::GetS1Size()
{
    if (qLayout_ == CopySfaMtpSFALayout::TND) {
        s1Size_ = GetAxisNum(queryShape_, CopySfaMtpSFAAxis::T, qLayout_);
        return ge::GRAPH_SUCCESS;
    } else { // BSND
        s1Size_ = GetAxisNum(queryShape_, CopySfaMtpSFAAxis::S, qLayout_);
    }
    return ge::GRAPH_SUCCESS;
}

ge::graphStatus CopySfaMtpSFAInfoParser::GetKvStorageMode()
{
    if (kvLayout_ == CopySfaMtpSFALayout::PA_BSND) {
        kvStorageMode_ = CopySfaMtpKvStorageMode::PAGE_ATTENTION;
    } else {
        kvStorageMode_ = CopySfaMtpKvStorageMode::BATCH_CONTINUOUS;
    }
    return ge::GRAPH_SUCCESS;
}

ge::graphStatus CopySfaMtpSFAInfoParser::GetKvLayout()
{
    const map<string, CopySfaMtpSFALayout> layoutKVMap = {
        {"BSND",        CopySfaMtpSFALayout::BSND},
        {"PA_BSND",     CopySfaMtpSFALayout::PA_BSND},
        {"TND",         CopySfaMtpSFALayout::TND}
    };
    ...
    if (kvLayout_ != CopySfaMtpSFALayout::PA_BSND && qLayout_ != kvLayout_) {
    ...
    if (kvLayout_ == CopySfaMtpSFALayout::PA_BSND && keyDimNum != 4U) {
...
ge::graphStatus CopySfaMtpSFAInfoParser::GetS2SizeForBatchContinuous()
{
    if (kvLayout_ == CopySfaMtpSFALayout::BSND) { // BSND
        s2Size_ = GetAxisNum(keyShape_, CopySfaMtpSFAAxis::S, kvLayout_);
    } else if (kvLayout_ == CopySfaMtpSFALayout::TND) {
        s2Size_ = GetAxisNum(keyShape_, CopySfaMtpSFAAxis::T, kvLayout_);
    }
    return ge::GRAPH_SUCCESS;
}

ge::graphStatus CopySfaMtpSFAInfoParser::GetMaxBlockNumPerBatch()
{
...
ge::graphStatus CopySfaMtpSFAInfoParser::GetBlockSize()
{
    blockSize_ = GetAxisNum(keyShape_, CopySfaMtpSFAAxis::Bs, kvLayout_);
    return ge::GRAPH_SUCCESS;
}

ge::graphStatus CopySfaMtpSFAInfoParser::GetSparseBlockCount()
{
    sparseBlockCount_ = GetAxisNum(sparseIndicesShape_, CopySfaMtpSFAAxis::K, qLayout_);

    return ge::GRAPH_SUCCESS;
}

ge::graphStatus CopySfaMtpSFAInfoParser::GetS2SizeForPageAttention()
{
...
ge::graphStatus CopySfaMtpSFAInfoParser::GetS2Size()
{
    if (kvStorageMode_ == CopySfaMtpKvStorageMode::BATCH_CONTINUOUS) {
...
ge::graphStatus CopySfaMtpSFAInfoParser::GetValueHeadDim()
{
    vHeadDim_ = GetAxisNum(valueShape_, CopySfaMtpSFAAxis::D, kvLayout_);
    return ge::GRAPH_SUCCESS;
}

ge::graphStatus CopySfaMtpSFAInfoParser::GetRopeHeadDim()
{
    ropeHeadDim_ = GetAxisNum(queryRopeShape_, CopySfaMtpSFAAxis::D, qLayout_);
    return ge::GRAPH_SUCCESS;
}

ge::graphStatus CopySfaMtpSFAInfoParser::GetQueryAndOutLayout()
{
    const map<string, pair<CopySfaMtpSFALayout, CopySfaMtpSFALayout>> layoutMap = {
        {"BSND",        {CopySfaMtpSFALayout::BSND,    CopySfaMtpSFALayout::BSND}},
        {"TND",         {CopySfaMtpSFALayout::TND,     CopySfaMtpSFALayout::TND }},
    };
...
ge::graphStatus CopySfaMtpSFAInfoParser::GetTopkLayout()
{
    topkLayout_ = qLayout_;
    return ge::GRAPH_SUCCESS;
}

ge::graphStatus CopySfaMtpSFAInfoParser::GetN1Size()
{
    n1Size_ = GetAxisNum(queryShape_, CopySfaMtpSFAAxis::N, qLayout_);
    return ge::GRAPH_SUCCESS;
}

ge::graphStatus CopySfaMtpSFAInfoParser::GetN2Size()
{
    n2Size_ = GetAxisNum(keyShape_, CopySfaMtpSFAAxis::N, kvLayout_);
    return ge::GRAPH_SUCCESS;
}

void CopySfaMtpSFAInfoParser::SetSFAShape()
{
...
ge::graphStatus CopySfaMtpSFAInfoParser::GetGSize()
{
    if (n2Size_ != 0) {
...
ge::graphStatus CopySfaMtpSFAInfoParser::GetActualseqInfo()
{
...
void CopySfaMtpSFAInfoParser::GenerateInfo(CopySfaMtpSFATilingInfo &sfaInfo)
{
    ...
    sfaInfo.pageAttentionFlag = (kvStorageMode_ == CopySfaMtpKvStorageMode::PAGE_ATTENTION);
...
ge::graphStatus CopySfaMtpSFAInfoParser::Parse(CopySfaMtpSFATilingInfo &sfaInfo)
{
    if (context_ == nullptr) {
        OPS_LOG_E("CopySfaMtpAttention", "tiling context is nullptr!");
        return ge::GRAPH_FAILED;
    }
    OPS_LOG_FULL(DLOG_INFO, "CopySfaMtpAttention", "TilingContext: %s", CopySfaMtpDebugTilingContext(context_).c_str());

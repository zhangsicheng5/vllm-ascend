...
#include "fused_quant_lightning_indexer_manage_template_tiling_key.h"
#include "fused_quant_lightning_indexer_manage_kernel.h"
#include "fused_quant_lightning_indexer_manage_union.h"
#include "fused_quant_lightning_indexer_manage_workspace.h"

using namespace LIMtpC8Kernel;
    ...
    for (uint32_t batch = 0U; batch < batchSize; ++batch) {
        ...
        if (state == -1) {
            ...
            for (uint32_t source = static_cast<uint32_t>(cacheCount);
                ...
                LIMtpC8ServiceVec::SetWaitFlag<HardEvent::MTE3_V>(HardEvent::MTE3_V);
            }
            continue;
        }
...
#define LI_MTP_C8_COPY_TILING()                                                                                           \
    GET_TILING_DATA_WITH_STRUCT(FusedQuantLightningIndexerManageTilingData, tiling_data_in, tiling);                                      \
    const FusedQuantLightningIndexerManageTilingData *__restrict tiling_data = &tiling_data_in

#define INVOKE_LI_MTP_C8_TOPK(...)                                                                                        \
    do {                                                                                                               \
        LI_MTP_C8_COPY_TILING();                                                                                           \
        ...
        __gm__ uint8_t *unionPair0 =                                                                                    \
            user + MtpC8Workspace::Pair0Offset(GetBlockNum(), tiling_data->n1Size);                                      \
        __gm__ uint8_t *unionPair1 =                                                                                    \
            user + MtpC8Workspace::Pair1Offset(GetBlockNum(), tiling_data->n1Size, tiling_data->bSize);                  \
        ...
        if ASCEND_IS_AIV {                                                                                              \
            ...
            if ((GetBlockIdx() & 1U) == 0U) {                                                                          \
                ...
                if (!FinalizeStandardCounts(requestState, actualSeqLengthsQuery, missCount,                            \
                    ...
                    MtpC8Union::MtpC8MissUnion unionOp;                                                                     \
...
template <int DT>
__global__ __aicore__ void fused_quant_lightning_indexer_manage(
    ...
    __gm__ uint8_t *tiling)
{
    ...
    // C8 唯一实例化: query/key int8 + bf16 index weights (W_T), fp16 dequant scales
    INVOKE_LI_MTP_C8_TOPK(int8_t, int8_t, bfloat16_t);
#endif
}

#ifndef FUSED_QUANT_LIGHTNING_INDEXER_MANAGE_UNION_H
#define FUSED_QUANT_LIGHTNING_INDEXER_MANAGE_UNION_H
...
constexpr uint32_t LONG_SOURCE_MASK = LIMtpC8ServiceVec::LONG_SOURCE_MASK;
    ...
    LocalTensor<float> key, uint32_t sourceBase, uint32_t count)
{
    ...
    ShiftRight(keyBits, keyBits, LIMtpC8ServiceVec::SCORE_TAG_CLEAR_SHIFT, count);
    PipeBarrier<PIPE_V>();
    ShiftLeft(keyBits, keyBits, LIMtpC8ServiceVec::SCORE_TAG_CLEAR_SHIFT, count);
    PipeBarrier<PIPE_V>();
    Adds(keyBits.ReinterpretCast<int32_t>(), keyBits.ReinterpretCast<int32_t>(),
         static_cast<int32_t>(
             (sourceBase >> PACKED_SOURCE_BITS) &
             ((1U << LIMtpC8ServiceVec::INDEX_HIGH_BITS) - 1U)),
         count);
    PipeBarrier<PIPE_V>();
}
...
{
    const uint32_t high =
        (keyBits & ((1U << LIMtpC8ServiceVec::INDEX_HIGH_BITS) - 1U)) <<
        PACKED_SOURCE_BITS;
        ...
        LocalTensor<int32_t> unionDestinations)
    {
        ...
        if (count <= EVICT_CHUNK) {
            ...
            for (uint32_t scan = 0U; scan < chunks; ++scan) {
                ...
                LIMtpC8ServiceVec::MergeSort(accumulator, EVICT_CHUNK, chunkPair,

...
#include "fused_li_manage_mtp_c8_tiling.h"
#include <algorithm>
#include "../op_kernel/fused_li_manage_mtp_c8_template_tiling_key.h"
// s2 chunk 粒度与 kernel 侧单一来源(host 兼容 shim 见该头)。
#include "../op_kernel/fused_li_manage_mtp_c8_workspace.h"
...
ge::graphStatus FusedLiManageMtpC8Tiling::GetNpuInfo(FusedLiManageMtpC8TilingInfo &tilingInfo) const
{
    if (context_->GetNodeName() == nullptr) {
        OPS_LOG_E("FusedLiManageMtpC8", "opName got from TilingContext is nullptr.");
...
ge::graphStatus FusedLiManageMtpC8Tiling::GetTensorInfo(FusedLiManageMtpC8TilingInfo &tilingInfo) const
{
...
ge::graphStatus FusedLiManageMtpC8Tiling::CheckDtype(const FusedLiManageMtpC8TilingInfo &tilingInfo) const
{
...
ge::graphStatus FusedLiManageMtpC8Tiling::CheckShape(FusedLiManageMtpC8TilingInfo &tilingInfo) const
{
    const auto &op = tilingInfo.opParamInfo;
...
ge::graphStatus FusedLiManageMtpC8Tiling::ParseAndCheck(FusedLiManageMtpC8TilingInfo &tilingInfo)
{
...
ge::graphStatus FusedLiManageMtpC8Tiling::DoTiling(FusedLiManageMtpC8TilingInfo *tilingInfo)
{
    auto ascendcPlatform = platform_ascendc::PlatformAscendC(tilingInfo->platformInfo);
    uint32_t aivNum = ascendcPlatform.GetCoreNumAiv();
    uint32_t aicNum = ascendcPlatform.GetCoreNumAic();
    ...
    if (mtp_) {
        ...
        // fp16 elements, 每核单缓冲)。与 kernel.h Init / MtpC8Workspace 保持同序。
...
static ge::graphStatus TilingPrepareForFusedLiManageMtpC8(gert::TilingParseContext * /* context */)
{
    return ge::GRAPH_SUCCESS;
}

ge::graphStatus TilingForFusedLiManageMtpC8(gert::TilingContext *context)
{
    OPS_ERR_IF(context == nullptr,
               OPS_REPORT_VECTOR_INNER_ERR("FusedLiManageMtpC8", "Tiling context is null."),
               return ge::GRAPH_FAILED);
    FusedLiManageMtpC8TilingInfo liInfo;
    FusedLiManageMtpC8Tiling liTiling(context, true);
...
IMPL_OP_OPTILING(FusedLiManageMtpC8)
    .Tiling(TilingForFusedLiManageMtpC8)
    .TilingParse<FusedLiManageMtpC8CompileInfo>(TilingPrepareForFusedLiManageMtpC8);
